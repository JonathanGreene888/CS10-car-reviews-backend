  ...
  newUser.save(err => {
    if (err)
      return res.status(500).json({
        registerError: `An account with those credentials already exists, please sign in.`
      });
    ...
    JWT.sign(
      ...
      (err, token) => {
        if (err)
          return res.status(500).json({
            registerError: `There was an error when trying to generate a JWT for the user--please try again.`
          });
...
router.post("/login", (req, res) => {
  ...
  if (!credentials.email || !credentials.password)
    return res.status(400).send({
      loginError: "Please provide a username or email and a password to login."
    });
    ...
    .then(userRecord => {
      if (!userRecord || !userRecord.password)
        return res.status(404).json({
          loginError:
            "No user with that email address was found, please register or try re-entering your credentials."
        });
      ...
      if (bcrypt.compareSync(credentials.password, userRecord.password)) {
        JWT.sign(
          ...
          (err, decoded) => {
            if (err)
              return res.status(500).json({
                loginError: `There was an error when trying to generate a JWT for the user--please try again.`
              });
        ...
        res.status(401).json({
          loginError: `The password you provided didn't match the one stored in our database, please try again`
        });
    })
    .catch(err => {
      console.warn(err);
      res.status(500).json({
        loginError: `There was an error when trying to generate a JWT for the user--please try again. authRouter: 63`
      });
    });
});
